...
| `_workspace/docs/` | 워크스페이스 자체의 사용법과 운영 문서 보관 |
...
커스텀 명령어는 `_workspace/commands/` 에 Markdown 파일로 정의한다. 예를 들어 `session-handoff.md` 가 있으면 사용자는 `/session-handoff` 형식으로 해당 명령어를 호출할 수 있다.
