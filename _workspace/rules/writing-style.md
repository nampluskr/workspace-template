...
6. [수식 작성](#6-수식-작성)
7. [코드 블록](#7-코드-블록)
8. [이미지 및 Figure](#8-이미지-및-figure)
9. [테이블](#9-테이블)
10. [Jupyter Notebook 규칙](#10-jupyter-notebook-규칙)
11. [Jupyter Book (MyST) 규칙](#11-jupyter-book-myst-규칙)
12. [참고문헌](#12-참고문헌)
...
요약은 제목 바로 아래, 메타정보 블록 위에 위치한다. 1줄~2줄 plain text로 작성하며 별도 레이블(`**요약:**` 등)을 붙이지 않는다.

### 2.4 목차

목차는 구분선(`---`) 다음에 `**목차**` 굵은 글씨 레이블로 시작한다. 섹션은 번호 목록 + 바로가기 링크로 작성한다. 서브섹션과 H4 세부항목은 목차에 포함하지 않는다.
...
파일명 또는 메타정보에서 날짜는 `YYYY-MM-DD` 형식을 사용한다.

```
2026-06-05_meeting-summary.md
```
...
본문은 서술체(`~이다`, `~한다`)를 사용한다. 경어체(`~입니다`, `~합니다`), 이모지, 불필요한 강조(볼드, 이탤릭)의 남용은 금지한다.
...
H2와 H3에는 번호를 붙인다. 번호와 제목 사이는 공백 1칸으로 구분하며, H4부터는 번호를 붙이지 않는다.
...
alt text는 이미지 내용을 영어로 간략히 기술한다. 캡션은 이미지 바로 다음 줄에 굵은 글씨로 작성하며 상세 설명은 그 다음 줄에 별도 paragraph로 작성한다.
