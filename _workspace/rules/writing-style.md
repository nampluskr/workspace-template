...
- 일반 Markdown (.md): 온라인 문서 및 저장소 렌더링 대상 형식
- Jupyter Book / MyST Markdown: 대화형 노트북 및 웹북 생성 대상 형식
- LaTeX / PDF: 학술 인쇄 문서 생성 대상 형식
...
- `생성일시`: 문서 최초 작성일시이며 이후 변경 금지
- `수정일시`: 내용 변경 시마다 갱신
- `주제`: `_workspace/rules/subjects.md` 의 Subject 전체명을 사용하고 복수 주제는 쉼표로 구분

### 2.3. 요약

요약 작성 위치와 형식은 다음과 같다.

- 제목 바로 아래, 메타정보 블록 위에 위치.
- 1줄~2줄 plain text로 작성하며 별도 레이블(`**요약:**` 등)을 붙이지 않는다.

### 2.4. 목차

목차 작성 위치와 형식에 관한 규칙은 다음과 같다.

- 구분선(`---`) 다음에 `**목차**` 굵은 글씨 레이블 시작
- 섹션은 번호 목록 + 바로가기 링크로 작성한다.
- 제목과 서브섹션과 H4 세부항목은 목차에 포함하지 않는다.
...
# 분리 필요 (서로 다른 항목)
- 제목 바로 아래, 메타정보 블록 위에 위치
- 1줄~2줄 plain text로 작성

# 유지 (하나의 개념을 연속 서술)
참고문헌은 문서의 마지막 섹션으로 배치한다. 논문의 References에 해당하며 본문에서 인용한 모든 출처를 나열한다.
```

### 2.7. 도입 문장 규칙

테이블, 숫자 목록, 불릿 목록 바로 앞에는 반드시 도입 문장을 1문장 이상 삽입한다. 도입 문장은 나열 항목의 맥락과 존재 이유를 제시하며, "다음과 같다"와 같이 내용 없이 목록을 연결만 하는 표현은 사용하지 않는다.

- 적용 대상: 테이블(`|...|`), 숫자 목록(`1.`, `2.`), 불릿 목록(`-`, `*`)
- 생략 불가: 섹션 헤더 바로 다음에 목록이나 테이블이 위치하는 경우 포함
- 코드 블록(```` ``` ````): 코드 블록 앞의 도입 문장은 권장하지만 필수가 아님
...
수식에서 자주 혼용되는 LaTeX 명령어의 통일 표기 기준은 다음과 같다.
...
- 셀 내용은 개조식으로 작성하며 긴 문장은 본문에서 설명한다
